...
class MicroWebSrv2:
    ...
    def LoadModule(modName):
        ...
        if modName in MicroWebSrv2._modules:
            raise MicroWebSrv2Exception(
                'Module "%s" is already loaded.' % modName)
        try:
    ...
    def StartInPool(self, asyncSocketsPool):
        if not isinstance(asyncSocketsPool, XAsyncSocketsPool):
            raise ValueError(
                '"asyncSocketsPool" must be a XAsyncSocketsPool class.')
        if self._xasSrv:
        ...
        try:
            ...
            raise MicroWebSrv2Exception(
                'Cannot bind server on %s:%s.' % self._bindAddr)
        self._xasSrv.OnClientAccepted = self._onSrvClientAccepted
        self._xasSrv.OnClosed = self._onSrvClosed
        self.Log('Server listening on %s:%s.' %
                 self._bindAddr, MicroWebSrv2.INFO)
    ...
    def StartManaged(self, parllProcCount=1, procStackSize=0):
        ...
        try:
            ...
            raise ValueError(
                '"procStackSize" of %s is not correct (%s).' % (procStackSize, ex))
        self._xasPool = XAsyncSocketsPool()
        try:
            self.StartInPool(self._xasPool)
            try:
                self.Log(
                    'Starts the managed pool to wait for I/O events.', MicroWebSrv2.INFO)
                self._xasPool.AsyncWaitEvents(threadsCount=parllProcCount)
            except:
                raise MicroWebSrv2Exception(
                    'Not enough memory to start %s parallel processes.' % parllProcCount)
        except Exception as ex:
            self.Stop()
            raise ex
        finally:
    ...
    def _validateChangeConf(self, name='Configuration'):
        if self._xasSrv:
            raise MicroWebSrv2Exception(
                '%s cannot be changed while the server is running.' % name)
    ...
    def EnableSSL(self, certFile, keyFile, caFile=None):
        ...
        if not hasattr(ssl, 'SSLContext'):
            raise MicroWebSrv2Exception(
                'Unable to use SSL (implementation not supported).')
        ...
        try:
            ...
            raise ValueError(
                '"certFile" and "keyFile" must indicate the valid certificate and key files.')
    ...
    def MaxRequestContentLength(self, value):
        if not isinstance(value, int) or value <= 0:
            raise ValueError(
                '"MaxRequestContentLength" must be a positive integer.')
    ...
    def BindAddress(self, value):
        if not value or len(value) != 2 or \
           ...
           not isinstance(value[1], int):
            raise ValueError(
                '"BindAddress" must be a tuple of (ipAddr, tcpPort).')
    ...
    def RequestsTimeoutSec(self, value):
        if not isinstance(value, int) or value <= 0:
            raise ValueError(
                '"RequestsTimeoutSec" must be a positive integer.')
